...
export default async function handler(request: Request) {
  ...
  try {
    ...
    const fetchResponse = await fetch(
      `${SUPABASE_URL}/rest/v1/user_audio_files?id=eq.${audioId}&user_id=eq.${auth.userId}&select=*`,
      {
        headers: {
          'apikey': SUPABASE_SERVICE_KEY!,
    ...
    const deleteResponse = await fetch(
      `${SUPABASE_URL}/rest/v1/user_audio_files?id=eq.${audioId}&user_id=eq.${auth.userId}`,
      {
        method: 'DELETE',
        headers: {
          'apikey': SUPABASE_SERVICE_KEY!,
