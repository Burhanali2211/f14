...
export function FullscreenImageViewer({ 
  ...
  const constrainPosition = useCallback((x: number, y: number, currentZoom: number) => {
    ...
    if (scaledWidth <= viewportWidth) {
      ...
      const maxPan = (scaledWidth - viewportWidth) / 2;
      constrainedX = Math.max(-maxPan, Math.min(maxPan, x));
    }
    
    if (scaledHeight <= viewportHeight) {
      constrainedY = 0;
    } else {
      const maxPan = (scaledHeight - viewportHeight) / 2;
      constrainedY = Math.max(-maxPan, Math.min(maxPan, y));
    }
    ...
    if (e.touches.length === 1) {
      ...
      if (isDraggingRef.current && zoom > fitZoom * 1.1) {
        ...
        if (touchStartRef.current.distance > 0 && currentDistance > 0) {
          ...
          setZoom(newZoom);
          if (newZoom < zoom) {
            setPosition(pos => constrainPosition(pos.x, pos.y, newZoom));
          }
          userHasInteractedRef.current = true;
        }
