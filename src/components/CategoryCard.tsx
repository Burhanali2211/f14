...
export const CategoryCard = memo(function CategoryCard({ category, index = 0 }: CategoryCardProps) {
  ...
  const bgImageBlur = category.bg_image_blur ?? 12;
    ...
    // 2. Legacy Fiqh path if slug matches
    // 3. Default category path
    const targetPath = category.custom_path || 
                      (category.slug === 'fiqh-jafaria' ? '/fiqh' : `/category/${category.slug}`);
        ...
        <div className="relative z-10 p-3 sm:p-6 md:p-5 lg:p-6 flex flex-col h-full min-h-[160px] sm:min-h-[220px] md:min-h-[200px] lg:min-h-[220px]">
          ...
          <div className="flex-1 flex flex-col">
            ...
            <div className={`w-10 h-10 sm:w-16 sm:h-16 md:w-14 md:h-14 lg:w-16 lg:h-16 rounded-lg sm:rounded-xl md:rounded-2xl ${iconBg} backdrop-blur-xl border-2 flex items-center justify-center mb-2 sm:mb-5 md:mb-4 lg:mb-5 group-hover:scale-110 group-hover:rotate-6 transition-all duration-500 ${
              ...
              <Icon className={`w-5 h-5 sm:w-8 sm:h-8 md:w-7 md:h-7 lg:w-8 lg:h-8 ${iconColor} transition-transform duration-500 group-hover:scale-110`} />
            </div>
            
            {/* Title - better contrast in light mode */}
            <h3 className={`text-base sm:text-2xl md:text-xl lg:text-2xl font-bold mb-1 sm:mb-3 md:mb-2 lg:mb-3 transition-colors duration-500 leading-tight ${
              theme === 'light'
