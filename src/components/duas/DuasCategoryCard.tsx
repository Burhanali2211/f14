...
export const DuasCategoryCard = memo(function DuasCategoryCard({ index = 0 }: { index?: number }) {
  ...
  return (
    <Link
      ...
      tabIndex={0}
      aria-label="Browse Duas and Ziyarat"
    >
      {/* Color overlay */}
      <div className={`absolute inset-0 bg-gradient-to-br transition-opacity duration-700 ${
        theme === 'light'
          ? 'from-violet-500/15 via-rose-500/8 to-primary/10'
          : 'from-violet-500/25 via-rose-500/12 to-primary/15'
      }`} />
      ...
      <div className="relative z-10 p-3 sm:p-6 md:p-5 lg:p-6 flex flex-col h-full min-h-[160px] sm:min-h-[220px] md:min-h-[200px] lg:min-h-[220px]">
        <div className="flex-1 flex flex-col">
          {/* Icon */}
          <div className={`w-10 h-10 sm:w-16 sm:h-16 md:w-14 md:h-14 lg:w-16 lg:h-16 rounded-lg sm:rounded-xl md:rounded-2xl bg-violet-500/15 dark:bg-violet-500/10 border-violet-500/30 dark:border-violet-500/20 backdrop-blur-xl border-2 flex items-center justify-center mb-2 sm:mb-5 md:mb-4 lg:mb-5 group-hover:scale-110 group-hover:rotate-6 transition-all duration-500 ${
            theme === 'light'
              ? 'shadow-md group-hover:shadow-lg'
              : 'shadow-lg group-hover:shadow-xl'
          }`}>
            <HandHeart className="w-5 h-5 sm:w-8 sm:h-8 md:w-7 md:h-7 lg:w-8 lg:h-8 text-violet-600 dark:text-violet-400 transition-transform duration-500 group-hover:scale-110" />
          </div>

          {/* Arabic title */}
          <h3 className={`text-sm sm:text-2xl md:text-xl lg:text-2xl font-bold mb-0.5 sm:mb-1 transition-colors duration-500 leading-tight font-arabic ${
            theme === 'light'
          ...
          <h4 className={`text-xs sm:text-xl md:text-lg lg:text-xl font-semibold mb-1 sm:mb-3 md:mb-2 lg:mb-3 transition-colors duration-500 leading-tight ${
            theme === 'light'
              ...
              : 'text-foreground/90 group-hover:text-violet-400'
          }`}>
            Duas & Ziyarat
          </h4>
          ...
          <p className={`text-[10px] sm:text-sm md:text-xs lg:text-sm line-clamp-2 leading-relaxed mb-2 sm:mb-4 md:mb-3 lg:mb-4 transition-colors duration-500 ${
            ...
            Ziyarat e Ashura, Dua e Kumail, Dua e Ahd, Taqibaat & more...
          </p>
        </div>
        ...
        <div className={`mt-auto pt-2 sm:pt-4 md:pt-3 lg:pt-4 border-t transition-colors duration-500 ${
          theme === 'light'
            ? 'border-border/60 group-hover:border-violet-500/60'
            : 'border-border/30 group-hover:border-violet-500/30'
        }`}>
          <div className="flex items-center justify-between">
            <span className={`text-[10px] sm:text-sm md:text-xs lg:text-sm font-semibold transition-colors duration-500 ${
              theme === 'light'
            ...
            <div className={`w-6 h-6 sm:w-9 sm:h-9 md:w-8 md:h-8 lg:w-9 lg:h-9 rounded-full flex items-center justify-center transition-all duration-500 group-hover:scale-110 group-hover:rotate-12 ${
              theme === 'light'
                ? 'bg-violet-500/20 group-hover:bg-violet-500/30 border border-violet-500/40 group-hover:border-violet-500/60'
                : 'bg-violet-500/10 group-hover:bg-violet-500/20 border border-violet-500/20 group-hover:border-violet-500/40'
            }`}>
              <ArrowRight className={`w-3 h-3 sm:w-4 sm:h-4 md:w-4 md:h-4 lg:w-5 lg:h-5 transition-transform duration-500 group-hover:translate-x-1 ${
                theme === 'light'
                  ? 'text-violet-600'
                  : 'text-violet-400'
              }`} />
      ...
      <div className={`absolute top-0 right-0 w-24 h-24 md:w-20 md:h-20 lg:w-24 lg:h-24 bg-gradient-to-bl rounded-bl-full opacity-0 group-hover:opacity-100 transition-opacity duration-700 blur-2xl ${
        theme === 'light'
          ? 'from-violet-500/30 via-violet-500/20 to-transparent'
