...
export default function CategoryFormPage() {
    ...
    const fetchCategory = async () => {
      ...
      if (!data) {
        ...
        setCategoryForm({
          ...
          custom_path: category.custom_path || '',
  ...
  const handleSave = async () => {
    ...
    if (!categoryForm.slug && !categoryForm.custom_path) {
      toast({
        title: 'Error',
        description: 'Either slug or custom path is required',
        variant: 'destructive',
      });
      return;
    }
    ...
    try {
      ...
      if (categoryImageFile) {
        ...
        const data = {
          ...
          custom_path: categoryForm.custom_path || null,
          ...
          <div className="bg-card rounded-2xl shadow-soft p-6 md:p-8 space-y-6">
            ...
            <div className="space-y-4">
              <h2 className="text-xl font-semibold text-foreground">Basic Information</h2>
                ...
                <div>
                    <Label htmlFor="cat-slug">Slug {!categoryForm.custom_path && '*'}</Label>
                    <Input
                      id="cat-slug"
                      ...
                      placeholder="e.g., naat"
                      className="mt-1"
                      disabled={!!categoryForm.custom_path}
                    />
                    <p className="text-xs text-muted-foreground mt-1">
                      {categoryForm.custom_path 
                        ? 'Disabled - using custom path instead'
                        : 'URL-friendly identifier. Link: /category/[slug]'}
                    </p>
                  </div>

                  <div>
                    <Label htmlFor="cat-path">Custom Path {!categoryForm.slug && '*'}</Label>
                    <Input
                      id="cat-path"
                      value={categoryForm.custom_path}
                      onChange={(e) => {
                        const path = e.target.value;
                        setCategoryForm(f => ({ 
                          ...f, 
                          custom_path: path,
                          slug: path ? '' : f.name.toLowerCase().replace(/\s+/g, '-')
                        }));
                      }}
                      placeholder="e.g., /fiqh"
                      className="mt-1"
                      disabled={!!categoryForm.slug && !categoryForm.custom_path}
                    />
                    <p className="text-xs text-muted-foreground mt-1">
                      {categoryForm.slug && !categoryForm.custom_path
                        ? 'Clear slug to use custom path instead'
                        : 'Direct link path (e.g., /fiqh opens directly, not /category/fiqh)'}
                    </p>
                  </div>
            ...
            <div className="flex items-center justify-end gap-3 pt-6 border-t">
              ...
              <Button
                  onClick={handleSave}
                  disabled={saving || uploadingImage || !categoryForm.name || (!categoryForm.slug && !categoryForm.custom_path)}
