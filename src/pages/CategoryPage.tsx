...
export default function CategoryPage() {
  ...
  const seoData = useMemo(() => {
    ...
    const description = category.description || `Browse the complete collection of ${category.name} - read lyrics, listen to audio, and watch video recitations. The #1 source for ${category.name} on Followers of 14. Free access to thousands of Islamic pieces online.`;
    const keywords = `${category.name}, ${category.name} lyrics, ${category.name} recitation, manqabat, islamic lyrics hub, Followers of 14, f14, read ${category.name} online, free recitation`;
    ...
    return {
      title: `${category.name} Lyrics & Recitations - Complete Collection | Followers of 14`,
      description,
      keywords,
            ...
            <div className="flex gap-3">
              <div className="inline-flex items-center bg-card rounded-xl p-1.5 border border-border/50 shadow-sm">
                <Button
                  ...
                  className={`h-12 w-12 sm:h-11 sm:w-auto sm:px-4 rounded-lg transition-all duration-200 ${viewMode === 'grid'
                ...
                <Button
                  ...
                  className={`h-12 w-12 sm:h-11 sm:w-auto sm:px-4 rounded-lg transition-all duration-200 ${viewMode === 'list'
                      ? 'bg-primary text-primary-foreground shadow-sm'
                      : 'hover:bg-secondary/50 text-muted-foreground'
                    }`}
