...
export default function QuranSurahPage() {
  ...
  const [charGap, setCharGap] = useState<number>(() =>
    readPref(LS_CHAR, 0.02, v => clampToArray(parseFloat(v), CHAR_GAPS, 0.02)),
  );
  ...
  useEffect(() => {
    ...
    setVerses(undefined);
    getSurahVerses(number).then(data => { 
      setVerses(data); 
      setVersesLoading(false); 
      
      // If URL has an anchor like #ayah-10, scroll to it after content loads
  ...
  const setChar    = (g: number) => { setCharGap(g);          localStorage.setItem(LS_CHAR,        String(g)); };
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="pt-12 pb-8 px-3 sm:px-5 max-w-2xl mx-auto">
        ...
        <div className="bg-card/40 backdrop-blur-sm rounded-2xl border border-border/40 p-4 sm:p-6 mt-2 mb-4 text-center">
          <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-primary/10 flex items-center justify-center mx-auto mb-2.5">
            <span className="font-arabic-heading text-lg sm:text-xl font-bold text-primary leading-none">
              {toArabicNumber(surah.number)}
            </span>
          </div>
          <h1 className="font-arabic-heading text-2xl sm:text-3xl md:text-4xl font-bold text-foreground mb-1 leading-tight">
            {surah.arabicName}
          </h1>
          <h2 className="text-base sm:text-lg font-bold text-foreground mb-0.5">
            {surah.englishName}
          </h2>
          <p className="text-xs text-muted-foreground mb-3 opacity-80">{surah.englishMeaning}</p>
          <div className="flex items-center justify-center gap-1.5 flex-wrap">
            <span className="px-2 py-0.5 rounded-lg bg-primary/10 text-primary text-[10px] sm:text-xs font-semibold">
              {toArabicNumber(surah.verseCount)} Verses
            </span>
            <span className={`px-2 py-0.5 rounded-lg text-[10px] sm:text-xs font-semibold ${
            ...
            <span className="px-2 py-0.5 rounded-lg bg-muted text-muted-foreground text-[10px] sm:text-xs font-semibold">
          ...
          <>
            ...
            <div>
              {verses.ayahs.map((ayah, idx) => (
                <div key={ayah.number} id={`ayah-${ayah.number}`} className="mb-4 sm:mb-6 scroll-mt-24">
                  {/* No separator between cards */}
                  <AyahDisplay
                    ayah={ayah}
                    surahNumber={number}
