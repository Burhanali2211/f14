...
export default async function handler(request) {
    ...
    try {
        if (pieceMatch) {
            ...
            const piece = await supabaseQuery('pieces', 'id,title,text_content,image_url,reciter,category:categories(name),imam:imams(name)', { column: 'id', value: pieceId });
            if (piece) {
            ...
            const category = await supabaseQuery('categories', 'name,description', { column: 'slug', value: categorySlug });
            if (category) {
            ...
            const imam = await supabaseQuery('imams', 'name,title,description', { column: 'slug', value: figureSlug });
            if (imam) {
...
//# sourceMappingURL=og-redirect.js.map